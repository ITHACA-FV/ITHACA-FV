...
double f3(DenseVector x)
{
	assert(x.rows() == 3);
	return (4 - 2.1*x(0)*x(0) + (1/3.)*x(0)*x(0)*x(0)*x(0))*x(0)*x(0) + x(0)*x(1) + (-4 + 4*x(1)*x(1))*x(1)*x(1)+x(2)*x(1);
}
...
void runExample()
{
    ...
    // Sample function
	auto x0_vec = linspace(0, 2, 10);
	auto x1_vec = linspace(0, 2, 10);
	...
	for(auto x0 : x0_vec_2)
	{
		for(auto x1 : x1_vec_2)
		{
            ...
            // Sample function at x
				...
				e_max.at(0) = std::max(e_max.at(0), std::abs(bspline1.eval(xf2) - y));
				e_max.at(1) = std::max(e_max.at(1), std::abs(bspline2.eval(xf2) - y));
				e_max.at(2) = std::max(e_max.at(2), std::abs(bspline3.eval(xf2) - y));
				e_max.at(3) = std::max(e_max.at(3), std::abs(pspline.eval(xf2) - y));

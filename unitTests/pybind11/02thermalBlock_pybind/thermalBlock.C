...
class tutorial02: public laplacianProblem
{
    public:
        tutorial02(int argc, char* argv[])
            ...
            S(_S())
        {
            para = ITHACAparameters::getInstance(_mesh(),
                             _runTime());
        }
        ...
        void offlineSolve(std::string folder = "./ITHACAoutput/Offline/")
        {
            if (offline)
            {
...
int main(int argc, char* argv[])
{
    ...
    int NmodesTout = example.para->ITHACAdict->lookupOrDefault<int>("NmodesTout", 15);
    int NmodesTproj = example.para->ITHACAdict->lookupOrDefault<int>("NmodesTproj", 10);
...
PYBIND11_MODULE(thermalBlock, m)
{
    // bindings to Matrix class
    py::class_<tutorial02>(m, "tutorial02")
      .def(py::init([](
         std::vector<std::string> args) {
            std::vector<char *> cstrs;
            cstrs.reserve(args.size());
            for (auto &s : args) cstrs.push_back(const_cast<char *>(s.c_str()));
            return std::unique_ptr<tutorial02>(
               new tutorial02(cstrs.size(), cstrs.data()));
         } 
      ),
      py::arg("args")=std::vector<std::string> {} )
      .def("offlineSolve",&tutorial02::offlineSolve);
}
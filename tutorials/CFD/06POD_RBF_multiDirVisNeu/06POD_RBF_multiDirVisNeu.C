...
class tutorial06 : public SteadyNSTurbNeu
{
    public:
        explicit tutorial06(int argc, char* argv[])
            :
            SteadyNSTurbNeu(argc, argv),
            U(_U()),
            p(_p()),
            nut(_nut())
        {}
        ...
        PtrList<volVectorField> neuLiftField;
...
int main(int argc, char* argv[])
{
    ...
    {
        Eigen::MatrixXd tmpmu(example.mu.rows(), DirNum);
        for (label i = 0; i < example.mu.rows(); i++)
        {
            for (label j = 0; j < DirNum; j++)
            {
    ...
    forAll (example.NeumannFields, i)
    {
        example.NeumannFields.set(i, new vectorField
            (example.neuLiftField[i].boundaryField()[example.outletIndex(0, 0)].snGrad()));
    }
    ...
    for (label k = 0; k < par_online.rows(); k++)
    {
        ...
        pod_rbf.solveOnlineSUP(velNow, neuVal);
...
/// for the full order steady NS problem , <SteadyNSTurbNeu.H> is the child of <steadyNS.H>
/// and it is the class for the full order steady NS turbulent problem.
/// Finally <reducedSteadyNS.H> and <ReducedSteadyNSTurbNeu.H> are for the construction of the reduced order problems
/// \until ReducedSteadyNSTurbNeu
///
/// \subsection classtutorial06 Definition of the tutorial06 class
/// We define the tutorial06 class as a child of the SteadyNSTurbNeu class.
...
/// \skipline projectSUP
///
/// Now we proceed to the ROM part of the tutorial, at first we construct an object of the class <ReducedSteadyNSTurbNeu.H>.
///
/// \skipline reduced
///
...
/// \until nuRec
///
/// After we carried out the online stage using an object of the turbulent class <ReducedSteadyNSTurbNeu.H>, now we will repeat

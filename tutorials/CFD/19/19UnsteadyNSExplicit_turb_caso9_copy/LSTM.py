...
POD_u = np.load("alpha_u.npy")     # shape (T, r_u)
POD_p = np.load("alpha_p.npy")     # shape (T, r_p)
POD_phi = np.load("alpha_phi.npy") # shape (T, r_phi)
POD_O_nut = np.load("alpha_nut.npy") # shape (T, r_nut)
...
def create_sequences(X, Y, lookback, step=10):
    X_seq, Y_seq = [], []
...
# model = Sequential()
# model.add(LSTM(64, input_shape=(lookback, X_seq.shape[2]), return_sequences=False))
# model.add(Dense(Y_seq.shape[1]))  # output: r_nut

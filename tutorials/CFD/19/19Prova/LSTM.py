...
for i in range(Y_true_original.shape[1]): 
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"results/plots/rel_error_train.png")
    plt.show()
    plt.close()
...
for i in range(min(3, Y_true_original.shape[1])):  
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"results/plots/abs_error_train.png")
    plt.show()
    plt.close()
...
np.save("results/Y_val_pred.npy", Y_val_pred_orig)
np.save("results/Y_val_true.npy", Y_val_true_orig)
...
np.savetxt("results/validation_errors.csv", errors_array, delimiter=",", header="mse,mae", comments='')

# === ERRORE PER ISTANTE TEMPORALE (SNAPSHOT) ===
snapshot_mse = np.mean((Y_val_true_orig - Y_val_pred_orig)**2, axis=1)
np.save("results/snapshot_mse.npy", snapshot_mse)
...
plt.grid()
plt.tight_layout()
plt.savefig("results/plots/snapshot_mse.png")
plt.close()
...
for i in range(Y_val_true_orig.shape[1]):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"results/plots/rel_error_val.png")
    plt.close()
...
for i in range(min(3, Y_val_true_orig.shape[1])):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"results/plots/abs_error_val.png")
    plt.close()
...
class tutorial19: public UnsteadyNSExplicitTurb
{
    public:
        explicit tutorial19(int argc, char* argv[])
            :
            UnsteadyNSExplicitTurb(argc, argv),
            U(_U()),
            p(_p()),
            phi(_phi()),
            nut(_nut())
...
int main(int argc, char* argv[])
{
    ...
    Eigen::MatrixXd u_coeffs, p_coeffs, nut_coeffs;

    std::cout << "Ufield size: " << example.Ufield.size() << std::endl;
    std::cout << "Numero di modi POD U: " << example.Umodes.size() << std::endl;

    std::cout << "Pfield size: " << example.Pfield.size() << std::endl; 
    std::cout << "Numero di modi POD P: " << example.Pmodes.size() << std::endl;

    std::cout << "nutFields size: " << example.nutFields.size() << std::endl;
    std::cout << "Numero di modi POD nut: " << example.nutModes.size() << std::endl;

    u_coeffs = ITHACAutilities::getCoeffs(example.Ufield, example.Umodes);
    p_coeffs = ITHACAutilities::getCoeffs(example.Pfield, example.Pmodes);
    nut_coeffs = ITHACAutilities::getCoeffs(example.nutFields, example.nutModes);  

    std::cout << "u_coeffs size: " << u_coeffs.size() << std::endl;
    std::cout << "p_coeffs size: " << p_coeffs.size() << std::endl;
    std::cout << "nut_coeffs size: " << nut_coeffs.size() << std::endl;

    cnpy::save(u_coeffs, "./Coeffs/u_coeffs.npy");
    cnpy::save(p_coeffs, "./Coeffs/p_coeffs.npy");
    cnpy::save(nut_coeffs, "./Coeffs/nut_coeffs.npy");    
    ...
    // for (int i = 0; i<reduced.online_solution.size(); i++)
    // {
    //     cnpy::save(reduced.online_solution[i],"./reduced/sol_reduced_"+name(i)+".npy");
    // }

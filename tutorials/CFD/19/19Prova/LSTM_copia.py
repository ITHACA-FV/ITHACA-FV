...
os.makedirs("Copia/plots", exist_ok=True)
...
for i in range(Y_true_original.shape[1]): 
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"Copia/plots/rel_error_train.png")
    plt.show()
    plt.close()
...
for i in range(min(3, Y_true_original.shape[1])):  
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"Copia/plots/abs_error_train.png")
    plt.show()
    plt.close()
...
np.save("Copia/Y_val_pred.npy", Y_val_pred_orig)
np.save("Copia/Y_val_true.npy", Y_val_true_orig)

# === SALVA HISTORY (loss e val_loss) ===
np.save("Copia/training_history.npy", history.history)
...
plt.grid()
plt.tight_layout()
plt.savefig("Copia/plots/training_history.png")
plt.close()
...
np.savetxt("Copia/validation_errors.csv", errors_array, delimiter=",", header="mse,mae", comments='')

# === ERRORE PER ISTANTE TEMPORALE (SNAPSHOT) ===
snapshot_mse = np.mean((Y_val_true_orig - Y_val_pred_orig)**2, axis=1)
np.save("Copia/snapshot_mse.npy", snapshot_mse)
...
plt.grid()
plt.tight_layout()
plt.savefig("Copia/plots/snapshot_mse.png")
plt.close()
...
for i in range(Y_val_true_orig.shape[1]):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"Copia/plots/rel_error_val.png")
    plt.close()
...
for i in range(min(3, Y_val_true_orig.shape[1])):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"Copia/plots/abs_error_val.png")
    plt.close()
...
int main(int argc, char *argv[])
{
	...
	else {mu_f=ITHACAstream::readMatrix("./ITHACAoutput/mu_off_mat.txt");
	...
	prova.projectPPE("./Matrices",prova.NUproj,prova.NPproj,prova.NFluxproj,NPrec,prova.NTproj,NDec,prova.NCproj);
	...
	for (int i=ref_start; i<ref_start+tf; i++)
	{
		...
		err(1,c)=ITHACAutilities::error_fields(prova.Fluxfield[i],ridotto.FLUXREC[c]);
		err(2,c)=ITHACAutilities::error_fields(prova.Prec1field[i],ridotto.PREC1REC[c]);
		err(3,c)=ITHACAutilities::error_fields(prova.Prec4field[i],ridotto.PREC4REC[c]);
		err(4,c)=ITHACAutilities::error_fields(prova.Prec8field[i],ridotto.PREC8REC[c]);
		err(5,c)=ITHACAutilities::error_fields(prova.Tfield[i],ridotto.TREC[c]);
		err(6,c)=ITHACAutilities::error_fields(prova.Dec1field[i],ridotto.DEC1REC[c]);
		err(7,c)=ITHACAutilities::error_fields(prova.Dec3field[i],ridotto.DEC3REC[c]);
		err(8,c)=ITHACAutilities::error_fields(prova.PowerDensfield[i],ridotto.POWERDENSREC[c]);
		err(9,c)=ITHACAutilities::error_fields(prova.TXSFields[i],ridotto.TXSREC[c]);
		c++;
	}
...
// average temperature and total power at the last time instant, i.e rom.finalTime
	...
	// save SA online time
	std::ofstream diff_ton("./ITHACAoutput/t_onlineSA_"+name(Ntot)+"="+name(deltat_online));

...
{
    std::string message =
        "The input sample should have the right size. Here input size = " +
        std::to_string(_sample.size())
        + " while samplesSize = "
        + std::to_string(samplesSize);
    M_Assert(_sample.size() == samplesSize, message.c_str());
    M_Assert(sampleI < Nsamples,
             "The index of the input sample is bigger than the number of samples in the ensemble");
...
void ensemble::assignSamples(Eigen::MatrixXd _samples)
{
    ...
    {
        std::string message = "The input samples have a different size. Input size = " +
                              std::to_string(_samples.rows())
                              + " while samplesSize = "
                              + std::to_string(samplesSize);

...
template<>
List<Field <scalar> > ITHACAparallel::combineFields(GeometricField<scalar, fvPatchField, volMesh> & field)
{
	...
	for (int i = 0; i < N_BF; i++)
	{
		...
		{
			if(IndFaceLocal()[i].size()>0 && field.boundaryFieldRef()[i].type() != "zeroGradient" && field.boundaryFieldRef()[i].type() != "processor" )
			{
				GlobField[i+1][abs(IndFaceLocal()[i][k])-Start()[i]] = field.boundaryFieldRef()[i][k];
			}
		}
...
template<>
List<Field <vector> > ITHACAparallel::combineFields(GeometricField<vector, fvPatchField, volMesh> & field)
{

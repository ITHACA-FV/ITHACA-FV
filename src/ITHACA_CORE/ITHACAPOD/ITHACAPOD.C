...
void ITHACAPOD::getModes_sym(PtrList<volScalarField>& snapshotsP,
                         PtrList<volScalarField>& modes, PtrList<volScalarField>& modesSym, 
			PtrList<volScalarField>& modesAntiSym, bool podex, bool supex, bool sup, int nmodes)
{
    if (podex == 0)
    {
        ITHACAparameters para;
	...
	Eigen::MatrixXd SnapMatrixSym = 0.5*(SnapMatrix+SnapMatrix.transpose());
	Eigen::MatrixXd SnapMatrixAntiSym = 0.5*(SnapMatrix-SnapMatrix.transpose());
	//Eigen::MatrixXd SnapMatrixSym = sym(SnapMatrix);
	//Eigen::MatrixXd SnapMatrixAntiSym = skew(SnapMatrix);
	...
	Eigen::VectorXd VSym = 0.5*(V + V.transpose());
        Eigen::VectorXd VAntiSym = 0.5*(V - V.transpose());

        auto VM = V.asDiagonal();
        Eigen::MatrixXd _corMatrix = SnapMatrix.transpose() * VM * SnapMatrix;
	auto VMSym = VSym.asDiagonal();
        Eigen::MatrixXd _corMatrixSym = SnapMatrixSym.transpose() * VMSym * SnapMatrixSym;
	auto VMAntiSym = VAntiSym.asDiagonal();
        Eigen::MatrixXd _corMatrixAntiSym = SnapMatrixAntiSym.transpose() * VMAntiSym * SnapMatrixAntiSym;
        ...
        Eigen::VectorXd eigenValueseig;
        Eigen::MatrixXd eigenVectoreig;
	Eigen::VectorXd eigenValueseigSym;
        Eigen::MatrixXd eigenVectoreigSym;
	Eigen::VectorXd eigenValueseigAntiSym;
        Eigen::MatrixXd eigenVectoreigAntiSym;
        modes.resize(nmodes);
	modesSym.resize(nmodes);
	modesAntiSym.resize(nmodes);
        Info << "####### Performing the POD using EigenDecomposition for " <<
             snapshotsP[0].name() << " #######" << endl;
        int ncv = snapshotsP.size();
        Spectra::DenseSymMatProd<double> op(_corMatrix);
	//Spectra::DenseSymMatProd<double> op(_corMatrixSym);
	//Spectra::DenseSymMatProd<double> op(_corMatrixAntSym);
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> esEg;
	Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> esEgSym;
	Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> esEgAntiSym;
        ...
        else if (para.eigensolver == "eigen")
        {
            std::cout << "Using Eigen EigenSolver " << std::endl;
            esEg.compute(_corMatrix);
	    esEgSym.compute(_corMatrixSym);
	    esEgAntiSym.compute(_corMatrixAntiSym);
            M_Assert(esEg.info() == Eigen::Success,
                     "The Eigenvalue Decomposition did not succeed");
            eigenVectoreig = esEg.eigenvectors().real().rowwise().reverse().leftCols(
                                 nmodes);
            eigenValueseig = esEg.eigenvalues().real().reverse().head(nmodes);
	    eigenVectoreigSym = esEgSym.eigenvectors().real().rowwise().reverse().leftCols(
                                 nmodes);
            eigenValueseigSym = esEgSym.eigenvalues().real().reverse().head(nmodes);
	    eigenVectoreigAntiSym = esEgAntiSym.eigenvectors().real().rowwise().reverse().leftCols(
                                 nmodes);
            eigenValueseigAntiSym = esEgAntiSym.eigenvalues().real().reverse().head(nmodes);
        }
        ...
        Eigen::MatrixXd modesEig = (SnapMatrix * eigenVectoreig) *
                                   eigenValueseigLam.asDiagonal();
	Eigen::VectorXd eigenValueseigLamSym =
            eigenValueseigSym.real().array().cwiseInverse().abs().sqrt() ;
        Eigen::MatrixXd modesEigSym = (SnapMatrixSym * eigenVectoreigSym) *
                                   eigenValueseigLamSym.asDiagonal();
	Eigen::VectorXd eigenValueseigLamAntiSym =
            eigenValueseigAntiSym.real().array().cwiseInverse().abs().sqrt() ;
        Eigen::MatrixXd modesEigAntiSym = (SnapMatrixAntiSym * eigenVectoreigAntiSym) *
                                   eigenValueseigLamAntiSym.asDiagonal();
        List<Eigen::MatrixXd> modesEigBC;
        modesEigBC.resize(NBC);
	List<Eigen::MatrixXd> modesEigBCSym;
        modesEigBCSym.resize(NBC);
	List<Eigen::MatrixXd> modesEigBCAntiSym;
        modesEigBCAntiSym.resize(NBC);

        for (int i = 0; i < NBC; i++)
        {
            modesEigBC[i] = (SnapMatrixBC[i] * eigenVectoreig) *
                            eigenValueseigLam.asDiagonal();
	    modesEigBCSym[i] = (SnapMatrixBC[i] * eigenVectoreigSym) *
                            eigenValueseigLamSym.asDiagonal();
	    modesEigBCAntiSym[i] = (SnapMatrixBC[i] * eigenVectoreigAntiSym) *
                            eigenValueseigLamAntiSym.asDiagonal();
        }
    	...
    	for (label i = 0; i < modesSym.size(); i++)
        {
            volScalarField tmpSym(snapshotsP[0].name(), snapshotsP[0] * 0);
            Eigen::VectorXd vecSym = modesEigSym.col(i);
            tmpSym = Foam2Eigen::Eigen2field(tmpSym, vecSym);

            // Adjusting boundary conditions
            for (int k = 0; k < tmpSym.boundaryField().size(); k++)
            {
                ITHACAutilities::assignBC(tmpSym, k, modesEigBCSym[k].col(i));
            }

            modesSym.set(i, tmpSym);
        }

    	for (label i = 0; i < modesAntiSym.size(); i++)
        {
            volScalarField tmpAntiSym(snapshotsP[0].name(), snapshotsP[0] * 0);
            Eigen::VectorXd vecAntiSym = modesEigAntiSym.col(i);
            tmpAntiSym = Foam2Eigen::Eigen2field(tmpAntiSym, vecAntiSym);

            // Adjusting boundary conditions
            for (int k = 0; k < tmpAntiSym.boundaryField().size(); k++)
            {
                ITHACAutilities::assignBC(tmpAntiSym, k, modesEigBCAntiSym[k].col(i));
            }

            modesAntiSym.set(i, tmpAntiSym);
        }
	    ...
	    ITHACAstream::exportFields(modesSym, "./ITHACAoutput/POD/", snapshotsP[0].name()+"sym");
	    ITHACAstream::exportFields(modesAntiSym, "./ITHACAoutput/POD/", snapshotsP[0].name()+"antisym");
        }

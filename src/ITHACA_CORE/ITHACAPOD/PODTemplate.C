...
template<typename T>
PODTemplate<T>::PODTemplate(ITHACAPOD::Parameters* myParameters, const word& myfield_name) :
  ...
  runTime2(Foam::Time::controlDictName, ".", ithacaFVParameters->get_casenameData())
{
  f_field = new T(
    IOobject
    (
...
void PODTemplate<T>::define_paths()
{
  ...
  word pathHilbertSpace(ithacaFVParameters->get_pathHilbertSpace_fromHS(l_hilbertSp));
...
void PODTemplate<T>::computeMeanField()
{
  ...
  if (b_centeredOrNot)
  {
    ...
    ithacaFVParameters->set_meanEnergy( f_meanField->name() + "_" + l_hilbertSp, energyHilbertMean);
  }
  else
  {
    ITHACAutilities::setToZero(*f_meanField);
...
template<typename T>
void PODTemplate<T>::findTempFile(Eigen::MatrixXd* covMat, int* index1, int* index2)
{
...
template<typename T>
void PODTemplate<T>::deletePreviousTempCovMatrix_N(int* valI, int* valJ, int i, int j, int N)
{
...
template<typename T>
void PODTemplate<T>::diagonalisation(Eigen::MatrixXd& covMatrix, Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig)
{
  ...
  if (l_hilbertSp == "L2")
  {
    ithacaFVParameters->set_resolvedVaryingEnergy( f_field->name(), resolvedVaryingEnergy);
  }
  else
  {
    ithacaFVParameters->set_resolvedVaryingEnergy( f_field->name() + "_" + l_hilbertSp, resolvedVaryingEnergy);
  }
...
template<typename T>
PtrList<T> PODTemplate<T>::computeSpatialModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig)
{
  ...
  if (!exist_spatialModes)
  {
    ...
    for (label j = 0; j < l_nSnapshot; j++)
    {
      T snapshotj = *f_field;
      ithacaFVParameters->read_snapshot(snapshotj, l_startTime+j);
      if (ithacaFVParameters->get_DEIMInterpolatedField() == "nut" && l_hilbertSp == "dL2"){
...
template<typename T>
Eigen::MatrixXd PODTemplate<T>::computeTemporalModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig)
{
...
template<typename T>
Eigen::MatrixXd PODTemplate<T>::computeSimulationTemporalModes(PtrList<T>& f_spatialModes)
{
...
void PODTemplate<T>::lift(T& snapshot)
{
  ...
  // Specialisation
  template PODTemplate<volTensorField>::PODTemplate(ITHACAPOD::Parameters* myParameters, const word& myfield_name);
  ...
  template void PODTemplate<volTensorField>::appendMeanfieldtoSpatialModes(PtrList<volTensorField>& spatialModes);
  template void PODTemplate<volTensorField>::findTempFile(Eigen::MatrixXd* covMat, int* index1, int* index2);
  ...
  template void PODTemplate<volTensorField>::saveTempCovMatrix(Eigen::MatrixXd& covMatrix, int i, int j);
  template void PODTemplate<volTensorField>::deletePreviousTempCovMatrix_N(int* valI, int* valJ, int i, int j, int N);

  template Eigen::MatrixXd PODTemplate<volTensorField>::buildCovMatrix();
  template void PODTemplate<volTensorField>::addCovMatrixTriCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volTensorField>& snapshots,
    indexTri& indTri);
  template void PODTemplate<volTensorField>::addCovMatrixSquareCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volTensorField>& snapshots1,
    PtrList<volTensorField>& snapshots2,
    indexSquare& indSquare);
  template void PODTemplate<volTensorField>::diagonalisation(Eigen::MatrixXd& covMatrix, Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template PtrList<volTensorField> PODTemplate<volTensorField>::computeSpatialModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  ...
  template Eigen::MatrixXd PODTemplate<volTensorField>::computeTemporalModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template void PODTemplate<volTensorField>::getModes(PtrList<volTensorField>& spatialModes, Eigen::MatrixXd& temporalModes,
    Eigen::MatrixXd& temporalModesSimulation, Eigen::MatrixXd& covMatrix);
  template Eigen::MatrixXd PODTemplate<volTensorField>::computeSimulationTemporalModes(PtrList<volTensorField>& spatialModes);
  template void PODTemplate<volTensorField>::compute_lambda(Eigen::MatrixXd& temporalModes);
  template void PODTemplate<volTensorField>::lift(PtrList<volTensorField>& snapshots);
  template void PODTemplate<volTensorField>::lift(volTensorField& snapshot);

  // Specialisation
  template PODTemplate<volVectorField>::PODTemplate(ITHACAPOD::Parameters* myParameters, const word& myfield_name);
  ...
  template void PODTemplate<volVectorField>::appendMeanfieldtoSpatialModes(PtrList<volVectorField>& spatialModes);
  template Eigen::MatrixXd PODTemplate<volVectorField>::buildCovMatrix();
  template void PODTemplate<volVectorField>::addCovMatrixTriCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volVectorField>& snapshots,
    indexTri& indTri);
  template void PODTemplate<volVectorField>::addCovMatrixSquareCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volVectorField>& snapshots1,
    PtrList<volVectorField>& snapshots2,
    indexSquare& indSquare);
  template void PODTemplate<volVectorField>::diagonalisation(Eigen::MatrixXd& covMatrix, Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template PtrList<volVectorField> PODTemplate<volVectorField>::computeSpatialModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  ...
  template Eigen::MatrixXd PODTemplate<volVectorField>::computeTemporalModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template void PODTemplate<volVectorField>::getModes(PtrList<volVectorField>& spatialModes, Eigen::MatrixXd& temporalModes,
    Eigen::MatrixXd& temporalModesSimulation, Eigen::MatrixXd& covMatrix);
  template Eigen::MatrixXd PODTemplate<volVectorField>::computeSimulationTemporalModes(PtrList<volVectorField>& spatialModes);
  template void PODTemplate<volVectorField>::compute_lambda(Eigen::MatrixXd& temporalModes);
  template void PODTemplate<volVectorField>::lift(PtrList<volVectorField>& snapshots);
  template void PODTemplate<volVectorField>::lift(volVectorField& snapshot);

  // Specialisation
  template PODTemplate<volScalarField>::PODTemplate(ITHACAPOD::Parameters* myParameters, const word& myfield_name);
  ...
  template void PODTemplate<volScalarField>::appendMeanfieldtoSpatialModes(PtrList<volScalarField>& spatialModes);
  template Eigen::MatrixXd PODTemplate<volScalarField>::buildCovMatrix();
  template void PODTemplate<volScalarField>::addCovMatrixTriCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volScalarField>& snapshots,
    indexTri& indTri);
  template void PODTemplate<volScalarField>::addCovMatrixSquareCoeff(Eigen::MatrixXd& covMatrix,
    PtrList<volScalarField>& snapshots1,
    PtrList<volScalarField>& snapshots2,
    indexSquare& indSquare);
  template void PODTemplate<volScalarField>::diagonalisation(Eigen::MatrixXd& covMatrix, Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template PtrList<volScalarField> PODTemplate<volScalarField>::computeSpatialModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  ...
  template Eigen::MatrixXd PODTemplate<volScalarField>::computeTemporalModes(Eigen::VectorXd& eigenValueseig, Eigen::MatrixXd& eigenVectoreig);
  template void PODTemplate<volScalarField>::getModes(PtrList<volScalarField>& spatialModes, Eigen::MatrixXd& temporalModes,
    Eigen::MatrixXd& temporalModesSimulation, Eigen::MatrixXd& covMatrix);
  template Eigen::MatrixXd PODTemplate<volScalarField>::computeSimulationTemporalModes(PtrList<volScalarField>& spatialModes);
  template void PODTemplate<volScalarField>::compute_lambda(Eigen::MatrixXd& temporalModes);
  template void PODTemplate<volScalarField>::lift(PtrList<volScalarField>& snapshots);

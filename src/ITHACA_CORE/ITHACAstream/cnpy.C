...
cnpy::NpyArray load_the_npz_array(FILE* fp, uint32_t compr_bytes, uint32_t uncompr_bytes) {
    ...
    bool fortran_order;
    std::string number_type;
    cnpy::parse_npy_header(&buffer_uncompr[0], word_size, shape, fortran_order, number_type);
    ...
    const std::string fname, std::string order)
{
    ...
    typeNumber* data = arr.data<typeNumber>();
    ...
    if (order == "rowMajor")
    {
        for (size_t i = 0; i < arr.shape[0]; ++i)
        {
            ...
            {
                if (arr.number_type == "f4") mat(i, j) = (typeNumber) arr.data<float>()[arr.shape[1] * i + j];
                if (arr.number_type == "f8") mat(i, j) = (typeNumber) arr.data<double>()[arr.shape[1] * i + j];
                if (arr.number_type == "f16") mat(i, j) = (typeNumber) arr.data<long double>()[arr.shape[1] * i + j];
                if (arr.number_type == "i4") mat(i, j) = (typeNumber) arr.data<short>()[arr.shape[1] * i + j];
                if (arr.number_type == "i8") mat(i, j) = (typeNumber) arr.data<int>()[arr.shape[1] * i + j];
                if (arr.number_type == "i16") mat(i, j) = (typeNumber) arr.data<long int>()[arr.shape[1] * i + j];
            }
        }
    }
    else if (order == "colMajor")
    {
        for (size_t i = 0; i < arr.shape[0]; ++i)
        {
            for (size_t j = 0; j < arr.shape[1]; ++j)
            {
                if (arr.number_type == "f4") mat(i, j) = (typeNumber) arr.data<float>()[arr.shape[0] * j + i];
                if (arr.number_type == "f8") mat(i, j) = (typeNumber) arr.data<double>()[arr.shape[0] * j + i];
                if (arr.number_type == "f16") mat(i, j) = (typeNumber) arr.data<long double>()[arr.shape[0] * j + i];
                if (arr.number_type == "i4") mat(i, j) = (typeNumber) arr.data<short>()[arr.shape[0] * j + i];
                if (arr.number_type == "i8") mat(i, j) = (typeNumber) arr.data<int>()[arr.shape[0] * j + i];
                if (arr.number_type == "i16") mat(i, j) = (typeNumber) arr.data<long>()[arr.shape[0] * j + i];
            }
        }
    }
                                        ...
                                        const std::string fname, std::string order)
{
    ...
    typeNumber* data = reinterpret_cast<typeNumber*>(arr.data<double>());
                                  ...
                                  const std::string fname)
{
    ...
    double* data = reinterpret_cast<double*>(d1.data<double>());
...
{
    std::vector<size_t> shape1 = std::vector<size_t> {mat.nonZeros()};
    std::vector<size_t> shape2 = std::vector<size_t> {mat.outerSize() + 1};

...
void LRSensitivity::buildSamplingSet(std::vector<std::string>& pdflist,Eigen::MatrixXd plist)
{
	for (int i=0; i<No_parameters; i++)
	{	
		MatX.col(i)=ITHACAsampling::samplingMC(pdflist[i],trainingRange(i,0),trainingRange(i,1), plist(i,0), plist(i,1), Npoints);
        }
...
void LRSensitivity::getBetas()
{
	...
	{std::cout<<"Statistics about inputs or output are not computed yet, nothing to do ..."<<std::endl;}
}
...
void LRSensitivity::assessQuality()
{
	...
	{
	std::cout<<"Linear regression coefficients are not computed yet, nothing to do ..."<<std::endl;	
	}
}

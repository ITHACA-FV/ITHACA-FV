...
reducedUnsteadyNSturb::reducedUnsteadyNSturb(unsteadyNSturb& FOMproblem)
{
	...
	newton_object_sup = newton_unsteadyNSturb_sup(Nphi_u + Nphi_p , Nphi_u + Nphi_p, FOMproblem);
	newton_object_PPE = newton_unsteadyNSturb_PPE(Nphi_u + Nphi_p , Nphi_u + Nphi_p, FOMproblem);
...
int newton_unsteadyNSturb_sup::operator()(const Eigen::VectorXd &x, Eigen::VectorXd &fvec) const
{
...
int newton_unsteadyNSturb_sup::df(const Eigen::VectorXd &x,  Eigen::MatrixXd &fjac) const
{
...
// Operator to evaluate the residual for the supremizer approach
int newton_unsteadyNSturb_PPE::operator()(const Eigen::VectorXd &x, Eigen::VectorXd &fvec) const
{
	Eigen::VectorXd a_dot(Nphi_u);
	Eigen::VectorXd a_tmp(Nphi_u);
	Eigen::VectorXd b_tmp(Nphi_p);
...
// Operator to evaluate the Jacobian for the supremizer approach
int newton_unsteadyNSturb_PPE::df(const Eigen::VectorXd &x,  Eigen::MatrixXd &fjac) const
{
...
// * * * * * * * * * * * * * * * Solve Functions  * * * * * * * * * * * * * //
void reducedUnsteadyNSturb::solveOnline_sup(Eigen::MatrixXd& vel_now, label startSnap)
{   
	...
	y.tail(Nphi_p) = ITHACAutilities::get_coeffs_ortho(Psnapshots[startSnap], Pmodes);
...
// * * * * * * * * * * * * * * * Solve Functions  * * * * * * * * * * * * * //
void reducedUnsteadyNSturb::solveOnline_PPE(Eigen::MatrixXd& vel_now, label startSnap)
{
    // Create and resize the solution vector
	y.resize(Nphi_u + Nphi_p, 1);
	y.setZero();

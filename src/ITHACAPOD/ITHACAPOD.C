...
void ITHACAPOD::getNestedSnapshotMatrix(PtrList<volVectorField>& snapshotsU, PtrList<volVectorField>& UModesGlobal, int Npar, int NnestedOut)
{
	...
	for(unsigned int i = 0; i < Npar; i++)
	{
    	ITHACAPOD::getWeightedModes(SnapMatrixNested[i], UModesNested[i],0, 0, 0, NnestedOut);       
	}
...
void ITHACAPOD::getNestedSnapshotMatrix(PtrList<volScalarField>& snapshotsP, PtrList<volScalarField>& PModesGlobal, int Npar, int NnestedOut)
{
    ...
    for(unsigned int i = 0; i < Npar; i++)
    {
        ...
        ITHACAPOD::getWeightedModes(SnapMatrixNested[i], PModesNested[i],0, 0, 0, NnestedOut);     
    }
...
void ITHACAPOD::getWeightedModes(PtrList<volVectorField>& snapshotsU, PtrList<volVectorField>& modes, bool podex, bool supex, bool sup, int nmodes)
{
    if (nmodes == 0)
    {
        nmodes = snapshotsU.size() - 2;
    ...
    if (podex == 0)
    {
        ...
        Eigen::MatrixXd modesEig = (SnapMatrix * eigenVectoreig) * eigenValueseigLam.asDiagonal()*eigenValueseigWeigted.asDiagonal();
        List<Eigen::MatrixXd> modesEigBC;
        modesEigBC.resize(NBC);
        ...
        for (int i = 0; i < NBC; i++)
        {
            modesEigBC[i] = (SnapMatrixBC[i] * eigenVectoreig) * eigenValueseigLam.asDiagonal()*eigenValueseigWeigted.asDiagonal();
        }
...
void ITHACAPOD::getWeightedModes(PtrList<volScalarField>& snapshotsP, PtrList<volScalarField>& modes, bool podex, bool supex, bool sup, int nmodes)
{
    if (nmodes == 0)
    {
        nmodes = snapshotsP.size() - 2;

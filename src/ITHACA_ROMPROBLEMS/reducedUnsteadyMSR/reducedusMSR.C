...
reducedusMSR::reducedusMSR(usmsrProblem& FOMproblem)
{
	...
	newton_object_fd=newton_usmsr_fd(Nphi_u+Nphi_p, Nphi_u+Nphi_p, FOMproblem); 
	newton_object_n=newton_usmsr_n(Nphi_flux+Nphi_prec1+Nphi_prec2+Nphi_prec3+Nphi_prec4+Nphi_prec5+Nphi_prec6+Nphi_prec7+Nphi_prec8, Nphi_flux+Nphi_prec1+Nphi_prec2+Nphi_prec3+Nphi_prec4+Nphi_prec5+Nphi_prec6+Nphi_prec7+Nphi_prec8, FOMproblem);
	newton_object_t=newton_usmsr_t(Nphi_T+Nphi_dec1+Nphi_dec2+Nphi_dec3, Nphi_T+Nphi_dec1+Nphi_dec2+Nphi_dec3, FOMproblem);
}
...
/// Neutronics terms
	...
	fvecn(i)=-F_dot(i)+lf(0,0)+pf(0,0)-af(0,0)+F3_1(i)+F3_2(i)+F3_3(i)+F3_4(i)+F3_5(i)+F3_6(i)+F3_7(i)+F3_8(i);
	}
...
/// Thermal terms
	...
	fvect(i)=-T_dot(i)-tt(0,0)+T1(i)+xsf(0,0)+dhs1(0,0)+dhs2(0,0)+dhs3(0,0);
	}		
	label pfvect=Nphi_T;
...
void reducedusMSR::solveOnline(Eigen::MatrixXd vel_now, Eigen::MatrixXd temp_now, Eigen::VectorXd mu_online, label startSnap)
{
	Info<<"\n Starting online stage...\n"<<endl;	
	y.resize(Nphi_u+Nphi_p,1); //for fd
	y.setZero();
	w.resize(Nphi_flux+Nphi_prec1+Nphi_prec2+Nphi_prec3+Nphi_prec4+Nphi_prec5+Nphi_prec6+Nphi_prec7+Nphi_prec8,1); //for n
	w.setZero();
	...
	w.head(Nphi_flux) = ITHACAutilities::get_coeffs(Fluxsnapshots[startSnap], Fluxmodes);
	pos_w+=Nphi_flux; 
	w.segment(pos_w, Nphi_prec1) = ITHACAutilities::get_coeffs(Prec1snapshots[startSnap], Prec1modes);
	pos_w+=Nphi_prec1; 
	w.segment(pos_w, Nphi_prec2) = ITHACAutilities::get_coeffs(Prec2snapshots[startSnap], Prec2modes);
	pos_w+=Nphi_prec2; 
	w.segment(pos_w, Nphi_prec3) = ITHACAutilities::get_coeffs(Prec3snapshots[startSnap], Prec3modes);
	pos_w+=Nphi_prec3; 
	w.segment(pos_w, Nphi_prec4) = ITHACAutilities::get_coeffs(Prec4snapshots[startSnap], Prec4modes);
	pos_w+=Nphi_prec4; 
	w.segment(pos_w, Nphi_prec5) = ITHACAutilities::get_coeffs(Prec5snapshots[startSnap], Prec5modes);
	pos_w+=Nphi_prec5; 
	w.segment(pos_w, Nphi_prec6) = ITHACAutilities::get_coeffs(Prec6snapshots[startSnap], Prec6modes);
	pos_w+=Nphi_prec6; 
	w.segment(pos_w, Nphi_prec7) = ITHACAutilities::get_coeffs(Prec7snapshots[startSnap], Prec7modes);
	pos_w+=Nphi_prec7; 
	w.segment(pos_w, Nphi_prec8) = ITHACAutilities::get_coeffs(Prec8snapshots[startSnap], Prec8modes);

	z.head(Nphi_T) = ITHACAutilities::get_coeffs(Tsnapshots[startSnap], Tmodes);
	pos_z+=Nphi_T;
	z.segment(pos_z,Nphi_dec1) = ITHACAutilities::get_coeffs(Dec1snapshots[startSnap], Dec1modes);
	pos_z+=Nphi_dec1;
	z.segment(pos_z,Nphi_dec2) = ITHACAutilities::get_coeffs(Dec2snapshots[startSnap], Dec2modes);
	pos_z+=Nphi_dec2;
	z.segment(pos_z,Nphi_dec3) = ITHACAutilities::get_coeffs(Dec3snapshots[startSnap], Dec3modes);
	...
	// Create vector to store temporal solution and save initial condition as first solution
    	...
    	Eigen::MatrixXd tmp_sol_n(Nphi_flux + Nphi_prec1 + Nphi_prec2+ Nphi_prec3+ Nphi_prec4+ Nphi_prec5+ Nphi_prec6+ Nphi_prec7+ Nphi_prec8+ 1, 1);
    	tmp_sol_n(0) = time;
	...
	tmp_sol_C(0)=time;
        ...
        tv0[0] = time;
	for (int k=1; k<tv0.size(); k++){tv0[k]=mu_online(k-1);}
	...
	while (time<finalTime+dt)
	{
		time=time+dt;
        	...
        	tv[0] = time;
		for (int k=1; k<tv.size(); k++) {tv[k]=mu_online(k-1);}	
        	for (label i = 0; i < Nphi_const; i++)
        	{
		...
		if (res_fd.norm()/y.norm() < 1e-5)
        	{
            	std::cout << green << "|F_fd(x)| = " << res_fd.norm()/y.norm() << " - Minimun reached in " << hnls_fd.iter << " iterations " << def << std::endl << std::endl;
        	}
        	else
        	{
            	std::cout << red << "|F_fd(x)| = " << res_fd.norm()/y.norm() << " - Minimun reached in " << hnls_fd.iter << " iterations " << def << std::endl << std::endl;
        	}
		if (res_n.norm()/w.norm() < 1e-5)
        	{
            	std::cout << green << "|F_n(x)| = " << res_n.norm()/w.norm() << " - Minimun reached in " << hnls_n.iter << " iterations " << def << std::endl << std::endl;
        	}
        	else
        	{
            	std::cout << red << "|F_n(x)| = " << res_n.norm()/w.norm() << " - Minimun reached in " << hnls_n.iter << " iterations " << def << std::endl << std::endl;
        	}
		if (res_t.norm()/z.norm() < 1e-5)
        	{
            	std::cout << green << "|F_t(x)| = " << res_t.norm()/z.norm() << " - Minimun reached in " << hnls_t.iter << " iterations " << def << std::endl << std::endl;
        	}
        	else
        	{
            	std::cout << red << "|F_t(x)| = " << res_t.norm()/z.norm() << " - Minimun reached in " << hnls_t.iter << " iterations " << def << std::endl << std::endl;
        	}
...
void reducedusMSR::reconstruct_fd(fileName folder, int printevery)
{
    ...
    for (label i = 0; i < online_solution_fd.size(); i++)
    {
	    ...
	    std::ofstream of(folder+"/"+name(counter2)+"/"+name(online_solution_fd[i](0)));
...
void reducedusMSR::reconstruct_n(fileName folder, int printevery)
{
    ...
    for (label i = 0; i < online_solution_n.size(); i++)
    {
	    ...
	    std::ofstream of(folder+"/"+name(counter2)+"/"+name(online_solution_n[i](0)));
            nextwrite += printevery;
            counter2 ++;
...
void reducedusMSR::reconstruct_t(fileName folder, int printevery)
{
	...
	if (counter == nextwrite)
	    ...
	    std::ofstream of(folder+"/"+name(counter2)+"/"+name(online_solution_t[i](0)));
            nextwrite += printevery;
            counter2 ++;
            TREC.append(T_rec);
...
void reducedusMSR::reconstruct_C(fileName folder, int printevery)
{
    ...
    for (label i = 0; i < online_solution_C.size(); i++)
    {
	    ...
	    std::ofstream of(folder+"/"+name(counter2)+"/"+name(online_solution_C[i](0)));
            nextwrite += printevery;
            counter2 ++;
	    vREC.append(v_rec);
	    DREC.append(D_rec);

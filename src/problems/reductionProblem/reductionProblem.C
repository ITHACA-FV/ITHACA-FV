...
std::vector<SPLINTER::RBFSpline> reductionProblem::getCoeffManifoldRBF(PtrList<volVectorField> snapshots, PtrList<volVectorField>& modes, word rbfBasis)
{
	Eigen::MatrixXd coeff = ITHACAutilities::get_coeffs_ortho(snapshots, modes);
	M_Assert(mu_samples.rows() == coeff.cols() * mu.rows() , "The dimension of the coefficient matrix must correspond to the constructed parameter matrix 'mu_samples'");
	...
	for (label i=0; i<mu_samples.cols(); i++)
	{
	    ...
	    if (isConst == 0)
	    {
		    muSampRefined.conservativeResize(muSampRefined.rows(), muSampRefined.cols()+1);
	...
	for (label i=0; i<coeff.rows(); i++)
	{
	    ...
	    if (rbfBasis == "GAUSSIAN")
			rbfsplines.push_back(SPLINTER::RBFSpline(samples, SPLINTER::RadialBasisFunctionType::GAUSSIAN));
		else if (rbfBasis == "THIN_PLATE")
			rbfsplines.push_back(SPLINTER::RBFSpline(samples, SPLINTER::RadialBasisFunctionType::THIN_PLATE_SPLINE));
		else if(rbfBasis == "MULTI_QUADRIC")
			rbfsplines.push_back(SPLINTER::RBFSpline(samples, SPLINTER::RadialBasisFunctionType::MULTIQUADRIC));
		else if(rbfBasis == "INVERSE_QUADRIC")
			rbfsplines.push_back(SPLINTER::RBFSpline(samples, SPLINTER::RadialBasisFunctionType::INVERSE_QUADRIC));
		else if(rbfBasis == "INVERSE_MULTI_QUADRIC")
			rbfsplines.push_back(SPLINTER::RBFSpline(samples, SPLINTER::RadialBasisFunctionType::INVERSE_MULTIQUADRIC));
		else
		{
	        std::cout << "Unknown string for rbfBasis. Valid types are 'GAUSSIAN', 'THIN_PLATE', 'MULTI_QUADRIC', 'INVERSE_QUADRIC', 'INVERSE_MULTI_QUADRIC'" << std::endl;
	        exit(0);
		}
	}
...
std::vector<SPLINTER::RBFSpline> reductionProblem::getCoeffManifoldRBF(PtrList<volScalarField> snapshots, PtrList<volScalarField>& modes, word rbfBasis)
{
	Eigen::MatrixXd coeff = ITHACAutilities::get_coeffs_ortho(snapshots, modes);
	M_Assert(mu_samples.rows() == coeff.cols() * mu.rows() , "The dimension of the coefficient matrix must correspond to the constructed parameter matrix 'mu_samples'");
	std::vector<SPLINTER::RBFSpline> rbfsplines;
...
std::vector<SPLINTER::BSpline> reductionProblem::getCoeffManifoldSPL(PtrList<volVectorField> snapshots, PtrList<volVectorField>& modes, label splDeg)
{
	Eigen::MatrixXd coeff = ITHACAutilities::get_coeffs_ortho(snapshots, modes);
	M_Assert(mu_samples.rows() == coeff.cols() * mu.rows() , "The dimension of the coefficient matrix must correspond to the constructed parameter matrix 'mu_samples'");
	...
	for (label i=0; i<coeff.rows(); i++)
	{
	    ...
	    SPLINTER::BSpline bspline = SPLINTER::BSpline::Builder(samples).degree(splDeg).build();
...
std::vector<SPLINTER::BSpline> reductionProblem::getCoeffManifoldSPL(PtrList<volScalarField> snapshots, PtrList<volScalarField>& modes, label splDeg)
{
	Eigen::MatrixXd coeff = ITHACAutilities::get_coeffs_ortho(snapshots, modes);
	M_Assert(mu_samples.rows() == coeff.cols() * mu.rows() , "The dimension of the coefficient matrix must correspond to the constructed parameter matrix 'mu_samples'");
	std::vector<SPLINTER::BSpline> bsplines;

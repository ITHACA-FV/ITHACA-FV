...
void msrProblem::truthSolve(List<scalar> mu_now) 
{
    ...
    volScalarField powerDens=(1-decbetaTot)*flux*SP+(decLam1*dec1+decLam2*dec2+decLam3*dec3);
...
void msrProblem::projectPPE(fileName folder, label NU, label NP,label NF, Eigen::VectorXi NPrec,label NT, Eigen::VectorXi NDec, label NC)
{

    if (NPrec.size()!=8 || NDec.size()!=3)
    {std::cout<<"The model assumes 8 groups of precursors and 3 of decay heat, check NDrec and NDec dimensions..."<<std::endl;
	exit(0);
    }
    NUmodes=NU;
    NPmodes=NP;
...
Eigen::MatrixXd msrProblem::diffusive_term(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(B_matrix, "B", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
// Export the matrix
    ...
    ITHACAstream::exportMatrix(K_matrix, "K", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");

    return K_matrix;
}

List < Eigen::MatrixXd > msrProblem::convective_term(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(C_matrix, "C", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
Eigen::MatrixXd msrProblem::mass_term(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(M_matrix, "M", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
Eigen::MatrixXd msrProblem::divergence_term(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(P_matrix, "P", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
List < Eigen::MatrixXd > msrProblem::div_momentum(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(G_matrix, "G", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
Eigen::MatrixXd msrProblem::laplacian_pressure(label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(D_matrix, "D", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
Eigen::MatrixXd msrProblem::pressure_BC1(label NUmodes, label NPmodes)
{
    ...
    ITHACAstream::exportMatrix(BC1_matrix, "BC1", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
List < Eigen::MatrixXd > msrProblem::pressure_BC2(label NUmodes, label NPmodes)
{
    ...
    // Export the matrix
        ITHACAstream::exportMatrix(BC2_matrix, "BC2", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
Eigen::MatrixXd msrProblem::pressure_BC3(label NUmodes, label NPmodes)
{
    ...
    for (label i = 0; i < P3_BC1size; i++)
    {
        ...
        ITHACAstream::exportMatrix(BC3_matrix, "BC3", "matlab", "./ITHACAoutput/Matrices/fluid_dynamics/");
...
List<Eigen::MatrixXd> msrProblem::laplacian_flux(label NFluxmodes, label NCmodes)
{
	...
	for (label j=0; j<NCmodes; j++)
	{
		...
		{
	               LF_matrix[i](j,k)=fvc::domainIntegrate(Fluxmodes[i]*fvc::laplacian(Dmodes[j],Fluxmodes[k])).value();
		}
	}
    }
	// Export the matrix
	ITHACAstream::exportMatrix(LF_matrix, "LF", "matlab", "./ITHACAoutput/Matrices/neutronics/");
...
Eigen::MatrixXd msrProblem::mass_flux(label NFluxmodes)
{
	...
	// Export the matrix
	ITHACAstream::exportMatrix(MF_matrix, "MF", "matlab", "./ITHACAoutput/Matrices/neutronics/");
...
List<Eigen::MatrixXd> msrProblem::prod_flux(label NFluxmodes, label NCmodes)
{
	...
	for (label i=0; i<PFsize; i++)
	{
		for (label j=0; j<NCmodes; j++)
		{
			...
			{
				PF_matrix[i](j,k)=fvc::domainIntegrate(Fluxmodes[i]*NSFmodes[j]*Fluxmodes[k]).value();
			}
		}
	}

	// Export the matrix
	ITHACAstream::exportMatrix(PF_matrix, "PF", "matlab", "./ITHACAoutput/Matrices/neutronics/");
...
List<Eigen::MatrixXd> msrProblem::abs_flux(label NFluxmodes, label NCmodes)
{
	...
	for (label i=0; i<AFsize; i++)
	{
		for (label j=0; j<NCmodes; j++)
		{
			...
			{
				AF_matrix[i](j,k)=fvc::domainIntegrate(Fluxmodes[i]*Amodes[j]*Fluxmodes[k]).value();
			}
		}
	}

	// Export the matrix
	ITHACAstream::exportMatrix(AF_matrix, "AF", "matlab", "./ITHACAoutput/Matrices/neutronics/");
	
	return AF_matrix;
}

Eigen::MatrixXd msrProblem::prec_source(label NFluxmodes, label NPrecmodes, int family)
{      
        int p=family; 
...
List<Eigen::MatrixXd> msrProblem::stream_term(label NUmodes, label NPrecmodes, int family)
{
	int p=family; 
	PtrList<volScalarField> Precmodes=choose_group("prec",p);
	...
	for (label i = 0; i < ST1size; i++)
       {
        for (label j = 0; j < ST2size; j++)
        {
            ...
            {
		ST_matrix[i](j, k) = fvc::domainIntegrate(Precmodes[i] * fvc::div(fvc::interpolate(Together[j]) & Together[j].mesh().Sf(), Precmodes[k])).value();
            }
        }
    }
...
Eigen::MatrixXd msrProblem::laplacian_prec(label NPrecmodes, int family)
{
	...
	for (label i=0; i<LPsize; i++)
	{
		...
		{
	               LP_matrix(i,j)=fvc::domainIntegrate(Precmodes[i]*fvc::laplacian(dimensionedScalar("1", dimless, 1),Precmodes[j])).value();
		}
	}
...
List<Eigen::MatrixXd> msrProblem::flux_source(label NFluxmodes, label NPrecmodes,label NCmodes, int family)
{
	int p=family;
        PtrList<volScalarField> Precmodes=choose_group("prec",p);
	...
	for (label j=0; j<NCmodes; j++)
	{
		...
		{
	               FS_matrix[i](j,k)=fvc::domainIntegrate(Precmodes[i]*NSFmodes[j]*Fluxmodes[k]).value();
	        }
	}
      }
...
List<Eigen::MatrixXd> msrProblem::stream_dec(label NUmodes, label NDecmodes, int decgroup)
{
	int g=decgroup; 
	...
	for (label i = 0; i < SD1size; i++)
       {
        for (label j = 0; j < SD2size; j++)
        {
            ...
            {
                SD_matrix[i](j, k) = fvc::domainIntegrate(Decmodes[i] * fvc::div(fvc::interpolate(Together[j]) & Together[j].mesh().Sf(), Decmodes[k])).value();
            }
        }
    }
...
Eigen::MatrixXd msrProblem::laplacian_dec(label NDecmodes, int decgroup)
{
	...
	for (label i=0; i<LDsize; i++)
	{
		...
		{
	               LD_matrix(i,j)=fvc::domainIntegrate(Decmodes[i]*fvc::laplacian(dimensionedScalar("1", dimless, 1),Decmodes[j])).value();
		}
	}
...
List<Eigen::MatrixXd> msrProblem::dec_fluxsource(label NFluxmodes, label NDecmodes, label NCmodes, int decgroup)
{
	int g=decgroup; 
	PtrList<volScalarField> Decmodes=choose_group("dec",g);
	...
	for (label j=0; j<NCmodes; j++)
	{
		for (label k=0; k<NFluxmodes; k++)
		{
	               DFS_matrix[i](j,k)=fvc::domainIntegrate(Decmodes[i]*SPmodes[j]*Fluxmodes[k]).value();
		}
	}
       }
...
Eigen::MatrixXd msrProblem::mass_temp(label NTmodes)
{
	...
	// Export the matrix
	ITHACAstream::exportMatrix(TM_matrix, "TM", "matlab", "./ITHACAoutput/Matrices/thermal/");
	...
	for (label i = 0; i < TS1size; i++)
       {
        for (label j = 0; j < TS2size; j++)
        {
            ...
            {
		TS_matrix[i](j, k) = fvc::domainIntegrate(TogetherT[i] * fvc::div(fvc::interpolate(Together[j]) & Together[j].mesh().Sf(), TogetherT[k])).value();
            }
        }
    }
	// Export the matrix
            ...
            ITHACAstream::exportMatrix(TS_matrix, "TS", "matlab", "./ITHACAoutput/Matrices/thermal/");
...
Eigen::MatrixXd msrProblem::laplacian_temp(label NTmodes)
{
	...
	for (label i=0; i<LTsize; i++)
	{
		...
		{
	               LT_matrix(i,j)=fvc::domainIntegrate(TogetherT[i]*fvc::laplacian(dimensionedScalar("1", dimless, 1),TogetherT[j])).value();
		}
	}

	// Export the matrix
		ITHACAstream::exportMatrix(LT_matrix, "LT", "matlab", "./ITHACAoutput/Matrices/thermal/");


	return LT_matrix;
}

List<Eigen::MatrixXd> msrProblem::temp_XSfluxsource(label NTmodes, label NFluxmodes, label NCmodes)
{
	...
	for (label j=0; j<NCmodes; j++)
	{
		for (label k=0; k<NFluxmodes; k++)
		{
	               TXS_matrix[i](j,k)=fvc::domainIntegrate(TogetherT[i]*TXSmodes[j]*Fluxmodes[k]).value();
		}
	}
       }

        ITHACAstream::exportMatrix(TXS_matrix, "TXS", "matlab", "./ITHACAoutput/Matrices/thermal/");

	return TXS_matrix;
}

List<Eigen::MatrixXd> msrProblem::temp_heatsource(label NTmodes, label NDecmodes, label NCmodes, int decgroup)
{      
        int  g=decgroup; 
	PtrList<volScalarField> Decmodes=choose_group("dec",g);	
	...
	for (label j=0; j<NCmodes; j++)
	{
		...
		{
	               THS_matrix[i](j,k)=fvc::domainIntegrate(TogetherT[i]*vmodes[j]*Decmodes[k]).value();
		}
	}
       }	
...
void msrProblem::readMSRfields()
{
	...
	volScalarField& SP=_SP();
	volScalarField& TXS=_TXS();
	volScalarField powerDens=flux*(1-_decbetaTot())*_SP()+_decLam1()*dec1+_decLam2()*dec2+_decLam3()*dec3;
	powerDens.rename("powerDens");
...
void msrProblem::msrcoeff(int& NC)
{
    ...
    {
        std::cout << "Constructing v RadialBasisFunction for mode " << i + 1 << std::endl;
    ...
    for (int i = 0; i < NCmodes; i++)
    {
        std::cout << "Constructing D RadialBasisFunction for mode " << i + 1 << std::endl;
    ...
    Eigen::MatrixXd Ncoeff_NSF = ITHACAutilities::get_coeffs_ortho(NSFFields, NSFmodes);
    ...
    for (int i = 0; i < NCmodes; i++)
    {
        std::cout << "Constructing NSF RadialBasisFunction for mode " << i + 1 << std::endl;
    ...
    for (int i = 0; i < NCmodes; i++)
    {
        std::cout << "Constructing A RadialBasisFunction for mode " << i + 1 << std::endl;
    ...
    Eigen::MatrixXd Ncoeff_SP = ITHACAutilities::get_coeffs_ortho(SPFields, SPmodes);
    ...
    for (int i = 0; i < NCmodes; i++)
    {
        std::cout << "Constructing  SP RadialBasisFunction for mode " << i + 1 << std::endl;
    ...
    Eigen::MatrixXd Ncoeff_TXS = ITHACAutilities::get_coeffs_ortho(TXSFields, TXSmodes);
    ...
    for (int i = 0; i < NCmodes; i++)
    {
        std::cout << "Constructing  TXS RadialBasisFunction for mode " << i + 1 << std::endl;
...
if (precInBool==true)
{
prec1.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec10.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec1));
prec2.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec20.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec2));
prec3.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec30.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec3));
prec4.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec40.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec4));
prec5.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec50.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec5));
prec6.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec60.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec6));
prec7.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec70.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec7));
prec8.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New(prec80.boundaryField()[precinIndex].type(),mesh.boundary()[precinIndex],prec8));
}
    T=T0;
    dec1=dec10;
    dec2=dec20;

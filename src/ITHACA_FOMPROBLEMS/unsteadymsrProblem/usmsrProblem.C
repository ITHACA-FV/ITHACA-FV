...
{   
	...
	#include "updateK.H"
	 ...
	 Info << "ExecutionTime = " << runTime.elapsedCpuTime() << " s"
       ...
       << nl << endl;
	powerDens=(1-decbetaTot)*flux*SP+(decLam1*dec1+decLam2*dec2+decLam3*dec3);
       		if (checkWrite(runTime))
		{
			...
			ITHACAstream::exportSolution(dec3, name(counter), "./ITHACAoutput/Offline/");
        		ITHACAstream::exportSolution(powerDens,name(counter), "./ITHACAoutput/Offline/");
			...
			std::ofstream ofk("./ITHACAoutput/Offline/"+name(counter)+"/"+name(Keff.value()));
			Ufield.append(U);
			Pfield.append(p);
			...
			PowerDensfield.append(powerDens);
			vFields.append(v);
			DFields.append(D);
			NSFFields.append(NSF);
			AFields.append(A);
			SPFields.append(SP);
			TXSFields.append(TXS);
			counter++;
...
void usmsrProblem::changePrecsBC()
{
	...
	prec1.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec1));
	prec2.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec2));
	prec3.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec3));
	prec4.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec4));
	prec5.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec5));
	prec6.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec6));
	prec7.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec7));
	prec8.boundaryFieldRef().set(precinIndex,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[precinIndex], prec8));
}
...
void usmsrProblem::computePrecsBC(int call)
{
	...
	bc_prec(0,call)=gSum(prec1.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam1.value()*tau.value());
	bc_prec(1,call)=gSum(prec2.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam2.value()*tau.value());
	bc_prec(2,call)=gSum(prec3.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam3.value()*tau.value());
	bc_prec(3,call)=gSum(prec4.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam4.value()*tau.value());
	bc_prec(4,call)=gSum(prec5.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam5.value()*tau.value());
	bc_prec(5,call)=gSum(prec6.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam6.value()*tau.value());
	bc_prec(6,call)=gSum(prec7.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam7.value()*tau.value());
	bc_prec(7,call)=gSum(prec8.boundaryField()[precoutIndex]*mesh.magSf().boundaryField()[precoutIndex])/gSum(mesh.magSf().boundaryField()[precoutIndex])*std::exp(-lam8.value()*tau.value());
